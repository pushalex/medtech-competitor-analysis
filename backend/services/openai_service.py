...
class OpenAIService:
    ...
    async def analyze_text(self, text: str) -> CompetitorAnalysis:
        """Анализ текста конкурента (брошюры, описания продуктов)"""
        logger.info("=" * 50)
        ...
        system_prompt = """Ты — эксперт-аналитик рынка медицинского оборудования (интервенционная хирургия и диагностика). 
...
Формат ответа (строго JSON):
{
    ...
    "unique_offers": ["УТП: например, уникальное покрытие стента или точность ОКТ", ...],
    "innovation_score": 0-10,
    "clinical_potential": "оценка пользы для врача и пациента",
    "recommendations": ["рекомендация 1", "рекомендация 2", ...],
    "summary": "Краткое экспертное резюме"
}

Важно:
- Оценивай технические характеристики (материалы, точность, совместимость).
- innovation_score: 10 — прорывная технология, 0 — устаревшее решение.
- Пиши на русском языке, используй медицинскую терминологию."""
    ...
    async def analyze_image(self, image_base64: str, mime_type: str = "image/jpeg") -> ImageAnalysis:
        """Анализ изображения (лендинг мед. прибора, скриншот каталога)"""
        logger.info("=" * 50)
        ...
        system_prompt = """Ты — эксперт по визуальному маркетингу в сфере MedTech. 
Проанализируй изображение (лендинг или продукт) и верни JSON.

Формат ответа (строго JSON):
{
    "description": "Что за оборудование изображено",
    "marketing_insights": ["инсайт по позиционированию", "инсайт по подаче"],
    "visual_style_score": 0-10, 
    "design_score": 0-10,
    "medical_clarity": "насколько понятно и профессионально подана тех. информация",
    "visual_style_analysis": "анализ цветовой схемы (стерильность, доверие, технологичность)",
    "recommendations": ["как улучшить визуализацию для врачей"]
}

Важно:
- design_score: оцени насколько современно и удобно представлен продукт.
- Оценивай использование анатомических рендеров и графиков.
                ...
                messages=[
                    {"role": "system", "content": system_prompt},
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Проанализируй это изображение конкурента с точки зрения маркетинга и дизайна для врачей:"
                            },
                            {
                                "type": "image_url",
                                "image_url": {
